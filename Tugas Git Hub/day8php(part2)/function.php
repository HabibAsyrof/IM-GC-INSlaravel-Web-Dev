...
function greetings($salamalaikum){
    echo "hallo " . $salamalaikum .  " selamat datang di garuda cyber <br>";
}
;
...
*/ 
// Code function di sini 
function reverseString($pandrom2){
    $kosong2 = "";
    $newpandrom2 = str_split($pandrom2);
    for ($i=count($newpandrom2)-1; $i >= 0 ; $i--) { 
        $kosong2.= $newpandrom2[$i];
    }
...
// Code function di sini 
function palindrome($pandrom){
    $newpandrom = str_split($pandrom);
    $kosong = "";
    for ($i=count($newpandrom)- 1 ; $i >= 0 ; $i--) { 
        $kosong .=$newpandrom[$i];
    }
    echo $kosong. "<br>";
    if($kosong == $pandrom){
